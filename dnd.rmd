...
```{r, echo=FALSE}
ep1 = sum(ac >= x1) / count
ep2 = sum(ac >= x1 & ac <= x2) / count
ex = sort(ac$AC)[[count * p3]]
ez = (ex - mean) / sd
```

Empirical Percentage of Points Above `r x1`: $$`r round(ep1*100, 1)`\%$$

Empirical Percentage of Points Between `r x1` & `r x2`: $$`r round(ep2*100, 1)`\%$$
