...
The mean of the data is `r round(mean, 2)`, and the standard deviation is `r round(sd, 2)`.

Mean:

$$\frac{\Sigma x}{n}\rightarrow\frac{`r mean * length(ac$AC)`}{`r length(ac$AC)`}=`r mean`$$
...
x = 10
z = (x - mean) / sd
percent = pnorm(z, lower.tail = FALSE)
```

Z-Score: $$Z = \frac{x-\mu}{\delta}\rightarrow
\frac{10-`r round(mean, 2)`}{`r round(sd, 2)`}=`r round(z, 2)`$$

Percent Above: $$P(`r round(z, 2)`< Z) = `r round(percent, 3)` = `r round(percent * 100, 1)`\%$$
